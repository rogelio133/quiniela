...
**Fecha:** 2026-07-08
**Estado:** ✅ Implementado (2026-07-08) — M1, M2, M3 y N10 completos. Verificado con `dotnet build` limpio (0 errores); sin verificación en navegador ni disparo del endpoint `/api/notify/check` en vivo en esta sesión.
...
| Empates | **Todos los empatados ganan** la insignia/medalla y su notificación. Si TODOS los miembros empatan (mejor = peor, incluye "nadie pronosticó" y salas de 1 miembro), **no se otorga ninguna** de las dos ese día. |
...
4. **Mejor del día** = todos los miembros con el máximo. **Peor del día** = todos los miembros con el mínimo.
5. Si `max == min` (todos empatados): **no se otorga nada** ese día.
...
**Orden sugerido:** M1 (independiente) → M2 → M3 → N10 (N10 y M3 dependen de M2).
...
public class DailyAwardService(IDbContextFactory<QuinielaDbContext> dbFactory)
{
    ...
    // Un día específico (lo usa N10 a las 21:30). Null si el día no otorga premio.
...
4. `max`/`min` sobre TODOS los miembros; si `max == min` el día se descarta. Si no, `BestUserIds` = empatados con `max`, `WorstUserIds` = empatados con `min`.

Notas:

- `GetCountsAsync` solo agrega: `Best += 1` por cada día en que el usuario está en `BestUserIds` (ídem `Worst`).
...
- [x] Un día con partidos finalizados y puntajes distintos produce exactamente un grupo de mejores y uno de peores
...
new("daily-best",  "🌞", "Mejor del día",
    "Fue quien más puntos sumó en un día de partidos. Una medalla 🏅 por cada día ganado.",
    AchievementCategory.Positive),
new("daily-worst", "🥴", "Peor del día",
    "Fue quien menos puntos sumó en un día de partidos (no pronosticar cuenta como 0). Una medalla 🏅 por cada día... sufrido.",
...
Con empates, los nombres se unen con " y " ("Rogelio y Pepe son los mejores del día 👑"); los empatados reciben cada uno su versión personal sin cambios. Nombres = `User.DisplayName`.
...
3. **Por cada sala:** `DailyAwardService.GetForDayAsync(poolId, día)`. Si devuelve null (todos empatados / sin premio): la sala se salta — **sus miembros no reciben nada de N10 ese día**.
4. **Por cada miembro** de la sala con premio: si está en `BestUserIds` → personal de mejor; si está en `WorstUserIds` → personal de peor; si no → anuncio con nombres. Un usuario nunca está en ambas listas (`max == min` ya descartó ese caso). URL: `/pools/{poolId}/achievements`.
...
- [x] Con empate, todos los empatados reciben su versión personal y el anuncio une los nombres
- [x] Día con todos empatados: nadie de esa sala recibe N10
