...
**Fecha:** 2026-07-17
**Estado:** 🚧 En implementación — X1, X2 y X5 completados (2026-07-17)
...
| X5 | Pase visual final de la página completa (criterio pendiente del doc 13) | X1–X4 | Bajo | ✅ Completado (2026-07-17, con X1–X2 integrados) |
...
- [x] Con X1–X4 integrados, correr el pase completo de la página (skill `verify`, patrón de los criterios del doc 13): **dark/light mode OK en toda la página; mobile 390px y desktop 1280px OK; 0 errores de consola; sin overflow horizontal.** Al pasar, marcar también el criterio pendiente del doc 13 (línea "Dark/light mode OK…") y el estado de RF8 en su tabla de módulos, apuntando a este doc. *(Corrido 2026-07-17 con la página tal como está integrada —X1 y X2; X3/X4 no tienen sección en este doc—, Playwright/admin/sala 1, 4 combinaciones (1280/390 × light/dark): 49/49 reveals en todas, 0 errores de consola, 0 overflow horizontal. El pase destapó dos fixes que quedaron dentro de X5: **(1) legibilidad sobre el balón fijo del layout** — los textos que viven directo sobre el fondo de la página (`fs-sec-kicker`, `fs-sec-title`, `fs-farewell`) cruzaban el balón al hacer scroll y se perdían (peor en 390px, donde el balón ocupa casi todo el ancho); se les dio un halo multi-capa `text-shadow` en `--q-bg` (funciona en ambos temas) y los botones outline del footer recibieron `background-color: var(--q-bg)`, verificado con screenshots antes/después en ambos temas. **(2) "🔁 Repetir ceremonia" ahora scrollea al inicio** — la ceremonia corre en el hero y el botón está en el footer, así que repetía sin que se viera; nuevo helper `quiniela.finalCeremony.scrollTop(smooth)` invocado desde `ReplayCeremony` (smooth salvo `reducedMotion`), con checks de `scrollY == 0` + ceremonia visible en desktop y mobile.)*
...
- [x] **X5:** pase visual completo de la página con las secciones nuevas — dark/light, 390px/1280px, 0 errores de consola, sin overflow horizontal — y doc 13 actualizado (criterio visual + estado RF8). *(Verificado con Playwright 2026-07-17, admin/sala 1, 16/16 checks en 4 combinaciones (1280 light/dark, 390 light/dark): 49/49 reveals, 0 errores de consola, sin overflow horizontal en las cuatro; screenshots de cada cabecera de sección posicionada sobre la zona del balón de fondo + footer en ambos temas, legibles tras el fix de halo (ver checklist X5); el scroll-al-inicio de "Repetir ceremonia" verificado en desktop y mobile. Doc 13 actualizado: criterio visual marcado y RF8 apuntando a este doc.)*
- [x] Sin migraciones nuevas ni librerías nuevas (solo canvas-confetti y html2canvas ya self-hosted); el gate de visibilidad de RF1 aplica igual a las secciones nuevas (vista previa admin incluida). *(X1–X5 solo tocaron markup/CSS/JS existentes — `Migrations/` y `wwwroot/lib/` sin cambios; todas las secciones nuevas viven dentro del render ya gateado de RF1 — el pase completo corrió como admin en modo vista previa con el banner visible.)*
